...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('flowVisualizer.generateFlowBlock', () => {
			...
			if (!code) {
				vscode.window.showWarningMessage('No code to not able to generate flow block.');
			...
			currentPanel ??= vscode.window.createWebviewPanel(
				...
				{ enableScripts: true, localResourceRoots: [context.extensionUri, vscode.Uri.joinPath(context.extensionUri, 'media')] }
			);

			currentPanel.webview.html ||= getInitialViewHtml(currentPanel.webview, context.extensionUri);
			const flowBlockDiv = getFlowBlockDiv(currentPanel.webview, context.extensionUri, code);
			currentPanel.webview.html = currentPanel.webview.html.replace(/(<div id="flow-block-div">).*(<\/div>)/s, '$1' + flowBlockDiv + '$2');
...
function getInitialViewHtml(webview: vscode.Webview, extensionUri: vscode.Uri) {
	const mainScriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'main.js'));
	const resetStyleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'reset.css'));
	const vscodeStyleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'vscode.css'));
	const flowVisualizerStyleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'flow-visualizer.css'));
	const flowVisualizerScriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'flow-visualizer.js'));
	const nonce = (() => {
		let text = '';
	...
	return `
		...
		<html lang="en">
			<head>
				...
				-->
				<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src https://* ${webview.cspSource}; script-src ${webview.cspSource} 'unsafe-inline'; style-src ${webview.cspSource};">
				<meta name="viewport" content="width=device-width, initial-scale=1.0">
				<link href="${resetStyleUri}" rel="stylesheet">
				<link href="${vscodeStyleUri}" rel="stylesheet">
				<link href="${flowVisualizerStyleUri}" rel="stylesheet">
				<title>Flow Visualizer</title>
			</head>
			<body>
				<div id="flow-block-div"></div>
				<script nonce="${nonce}" src="${flowVisualizerScriptUri}"></script>
				<script nonce="${nonce}" src="${mainScriptUri}"></script>
			</body>
		</html>`;
}

function getFlowBlockDiv(webview: vscode.Webview, extensionUri: vscode.Uri, sourceCode: string) {
	let flowblockDiv = '';
	...
	function processProgram(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			if (path.isProgram()) {
				...
				comments.push(`<table name="Start-block" class="outer-alignment-center padding-all-5px"><tr><td class="border-all-3px-solid-silver border-rounded-50percent backgroundcolor-yellow-200 padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'start.png'))}"></td></tr></table><!--/name="Start-block"-->`);
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px"><tr><td><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}"></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
		}
		else {
			if (path.isProgram()) {
				comments.push(`<table name="End-block" class="outer-alignment-center padding-all-5px"><tr><td class="border-all-3px-solid-silver border-rounded-50percent backgroundcolor-lime-500 padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'end.png'))}"></td></tr></table><!--/name="End-block"-->`);
	...
	function processIfStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isIfStatement()) {
				...
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}"></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
	...
	function processForStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isForStatement()) {
				comments.push(`<tr>`);
				comments.push(`<td class="backgroundcolor-orange-200 inner-alignment-center padding-all-2px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}"> or <img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'downtoright-arrow.png'))}"></td>`);
				comments.push(`<td class="backgroundcolor-orange-200 padding-all-5x"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'righttoup-arrow.png'))}"></td>`);
				comments.push(`</tr>`);
				comments.push(`</table><!--/name="ForStatement-block"-->`);
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><img name="${PathType.BreakStatement}" src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}" data-start="${path.node.start}" data-end="${path.node.end}"></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
			...
			if (path.parentPath?.isForStatement()) {
				if (path.key === Key.Update) {
					...
					comments.push(`<td class="backgroundcolor-orange-200 inner-alignment-middle padding-all-5px"><img name="${PathType.ContinueStatement}" src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'uptoleft-arrow.png'))}" data-start="${path.node.start}" data-end="${path.node.end}"></td>`);
					comments.push(`</tr><!--/name="ForStatement-Test-level"-->`);
					return addHtmlTagsToPathComments(path, isEnter, comments);
				}
				...
				if (path.key === Key.Body) {
					...
					comments.push(`<td class="backgroundcolor-orange-200 inner-alignment-middle padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'cycle-arrow.png'))}"></td>`);
					comments.push(`</tr><!--/name="ForStatement-Body-level"-->`);
					processExpressionStatement(path, isEnter, PathType.SingleLineStatement); //single-line expression
					processVariableStatement(path, isEnter, PathType.SingleLineStatement); //single-line expression
					return addHtmlTagsToPathComments(path, isEnter, comments);
	...
	function processForXStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isForXStatement()) {
				comments.push(`<tr>`);
				comments.push(`<td class="backgroundcolor-orange-200 inner-alignment-center padding-all-2px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}"> or <img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'downtoright-arrow.png'))}"></td>`);
				comments.push(`<td class="backgroundcolor-orange-200"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'righttoup-arrow.png'))}"></td>`);
				comments.push(`</tr>`);
			...
			if (path.parentPath?.isForXStatement()) {
				if (path.key === Key.Right) {
					...
					comments.push(`<td class="backgroundcolor-orange-200"><img name="${PathType.ContinueStatement}" src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'uptoleft-arrow.png'))}" data-start="${path.node.start}" data-end="${path.node.end}"></td>`);
					comments.push(`</tr><!--/name="ForXStatement-Test-level"-->`);
					return addHtmlTagsToPathComments(path, isEnter, comments);
				}
		...
		if (isEnter) {
			...
			if (path.parentPath?.isDoWhileStatement()) {
				if (path.key === Key.Test) {
					...
					comments.push(`<img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'down-arrow.png'))}"> or while (`);
					return addHtmlTagsToPathComments(path, isEnter, comments);
				}
			...
			if (path.parentPath?.isDoWhileStatement()) {
				if (path.key === Key.Test) {
					comments.push(`)<img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'right-arrow.png'))}">`);
			...
			if (path.parentPath?.isCallExpression()) {
				if (path.isIdentifier()) {
					...
					if (path.key === Key.Callee) {
						...
						comments.push(`<a href="#" data-function-id="${path.node.name}" onclick="toggleFunctionStatementBlock(this);changeFunctionStatementBlockParameterTooltips(${DoubleQuote}${path.node.name}${DoubleQuote}, ${DoubleQuote + JSON.stringify(callFunctionParameters).replace(/\\/g, '\\\\').replace(/"/g, '\\' + DoubleQuote) + DoubleQuote})">`);
						return addHtmlTagsToPathComments(path, isEnter, comments);
					}
				}
			}
	...
	function processContinueStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isContinueStatement()) {
				...
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><a href="#" onclick="moveOrStopFlowIcon(this, '${PathType.ContinueStatement}')"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'next-arrow.png'))}"></a></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
		}
	}
	...
	function processBreakStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isBreakStatement()) {
				comments.push(`</td></tr></table>`);
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><a href="#" onclick="moveOrStopFlowIcon(this, '${PathType.BreakStatement}')"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'end-arrow.png'))}"></a></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
		}
	}
	...
	function processThrowStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isThrowStatement()) {
				...
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><a href="#" onclick="toggleCatchStatementBlock(this)"><span class="tooltip"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'warning.png'))}"><div class="tooltip-text">Show Catch Statement Block</div></span></a></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
		}
	}
	...
	function processReturnStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isReturnStatement()) {
				...
				comments.push(`<table name="Next-block" class="outer-alignment-center padding-all-5px ${getUnreachableCode(path).className}"><tr><td><a href="#" onclick="hideAllFunctionStatementBlocks()"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'end-arrow.png'))}"></a></td></tr></table><!--/name="Next-block"-->`);
				return addHtmlTagsToPathComments(path, isEnter, comments);
			}
		}
	}
	...
	function processSwitchStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.isSwitchCase()) {
				...
				if (hasBreak && !isLast) {
					comments.push(`<td name="SwitchCase-Fallthrough-unit" class="backgroundcolor-gray-50 border-left-3px-solid-silver padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'uptoright-arrow.png'))}"></td><!--/name="SwitchCase-Fallthrough-unit"-->`);
				}
				...
				if (hasBreak && !isLast) {
					comments.push(`<tr>`);
					comments.push(`<td name="SwitchCase-Flow-unit" class="backgroundcolor-gray-50 border-top-3px-solid-silver inner-alignment-center padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'downtoright-arrow.png'))}"></td><!--/name="SwitchCase-Flow-unit"-->`);
					comments.push(`<td class="backgroundcolor-gray-50 padding-all-5px"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'righttoup-arrow.png'))}"></td>`);
					comments.push(`</tr>`);
				}
	...
	function processTryStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			...
			if (path.parentPath?.isTryStatement()) {
				//try-body
				if (path.key === Key.Block) {
					comments.push(`<a href="#" class="hidden-warning-sign" onclick="toggleCatchStatementBlock(this)"><span class="tooltip"><img src="${webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'warning.png'))}"><div class="tooltip-text">Show Catch Statement Block</div></span></a>`);
	...
	function processFunctionStatement(path: NodePath<Node>, isEnter: boolean) {
		...
		if (isEnter) {
			if (path.isFunctionDeclaration()) {
				comments.push(`<div data-function-id="${path.node.id?.name}" data-declare-function-parameter-json="${JSON.stringify((path.node.params as Node[]).map((parameter) => { return generate(parameter).code; })).replace(/\\/g, '\\\\').replace(/"/g, '\\' + DoubleQuote)}" class="hide">`);
				comments.push(`<a href="#" onclick="hideAllFunctionStatementBlocks()" class="close"></a>`);
	...
	flowblockDiv = convertToNodes(code);
